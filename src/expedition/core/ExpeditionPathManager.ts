...
export const MIN_EXPEDITION_WORK_TICK_CNT = 25;
...
export class ExpeditionPathManager {
    ...
    private static calculateTravelTime(roomDistance: number): number {
        ...
        const averageDistancePerRoom = MIN_EXPEDITION_WORK_TICK_CNT;
    ...
    // 验证Claimer是否能够完成远征任务
    static validateClaimerMission(roomDistance: number): {
        canComplete: boolean;
        travelTime: number;
        workTime: number;
        ...
        const canComplete = highSpeedWorkTime >= MIN_EXPEDITION_WORK_TICK_CNT;

        let recommendation = '';
        if (!canComplete) {
            const maxDistance = Math.floor((claimerLifetime - MIN_EXPEDITION_WORK_TICK_CNT) / (averageDistancePerRoom * 1.15));
